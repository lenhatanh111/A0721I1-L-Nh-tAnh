...
<h5 style="text-align: center; color: red">Khuyến cáo: Khai báo thông tin sai là vi phạm pháp luật Việt Nam và có thể xử
    lý hình sự</h5>
<form:form action="/declaration/update" method="post" modelAttribute="declaration" cssStyle="align-content: center">
    <table>
        <tr>
            <td>ID</td>
            <td></td>
        ...
        <tr>
            <td>ngay<form:select path="start_day" items="${start_day}"></form:select></td>
            <td>thang<form:select path="start_month" items="${start_month}"></form:select></td>
            <td>nam<form:select path="start_year" items="${start_year}"></form:select></td>
            <td>ngay<form:select path="end_day" items="${end_day}"></form:select></td>
            <td>thang<form:select path="end_month" items="${end_month}"></form:select></td>
            <td>nam<form:select path="end_year" items="${end_year}"></form:select></td>
        </tr>
        <tr>
